    ...
    public function pay_package(Request $request, $package_id)
    {
        try {
            ...
            if ($request->type == 1) {
                ...
                $new_packages_str = '';
                $new_packages_str .= $package_exist->id . ',';
                ...
                $response = $this->pay_to_pashaBank($total_amount, $this->userID, $return_type, $ip_address, 'packages', 0, $new_packages_str);
        ...
        try {
            ...
            if ($package) {
                ...
                $categories = Category::orderBy('name_' . App::getLocale())
                    ...
                    ->whereNull('country_id')->whereNull('deleted_by')
                    ->get();
                $currencies = Currency::orderBy('name')->select('id', 'name')->get();
    ...
    public function get_special_orders(Request $request, $a, $country_id)
    {
        try {
            ...
            if ($currency) {
                ...
                $currency_name = Currency::query()->select('name')->where('id', $currency->currency_id)->first()->name;;
...
//            }
    ...
    public function delete_special_order(Request $request, $country_id, $order_id)
    {
        try {
            $order_id = $request->order_id;
            $country_id = $request->country_id;
        ...
        try {
            ...
            foreach ($logs as $log) {
                if ($log->created_at instanceof \Carbon\Carbon) {
                    ...
                    $log->created_at = date('Y-m-d H:i', strtotime($log->created_at));
                }
            }
...
//                }
    ...
    public function courier_create_order_region(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'phone' => 'Telefon',
            'date' => 'Tarix',
            'courier_payment_type_id' => 'Kuryer çatdırılma ödəniş növü',
            'delivery_payment_type_id' => 'Xaricdən çatdırılma ödəniş növü',
