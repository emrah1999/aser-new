    ...
    public function callback_azericard_special(Request $request)
    {
        ...
        if ($data1['TRTYPE'] == 1) {
            ...
            switch ($action) {
                ...
                case '1':
                    {
                        $rc_description = "Dublikat əməliyyat aşkar edildi";

                    }
                    break;
                case '2':
                    {
                        $rc_description = "Tranzaksiya imtina edildi";

                    }
                    break;
                case '3':
                    {
                        $rc_description = "Tranzaksiya emal xətası baş verdi";

                    }
                    break;
                case '6':
                    {
                        $rc_description = "Rədd edilmiş əməliyyatın təkrarlanması";

                    }
                    break;
                case '7':
                    {
                        $rc_description = "Doğrulama xətası ilə əməliyyatın təkrarlanması";

                    }
                    break;
                case '8':
                    {
                        $rc_description = "Cavab verilmədən dayandırılmış əməliyyatın təkrarlanması";
            ...
            if ($payment_task) {
                ...
                $amount = $payment_task->amount;

                PaymentTask::where('id', $payment_id)->update([
                    'status' => 1,
                    'is_paid' => $is_paid,
                    'pan' => $data1['MASKED_CARD'],
                    'trtype2' =>$response,
                    'response_code_description' => $rc_description,
                    'response_rc' => $res_rc,
                    'response_rc_description' => $rc_description,
                    'response_str' => json_encode($data1)
                ]);
                ...
                if ($is_paid == 1) {
                    ...
                    $order_payments = SpecialOrderPayments::where('payment_key', $data['NONCE'])->orderBy('id', 'desc')->select('id', 'order_id')->first();
                    if ($order_payments) {
                        $order_id = $order_payments->order_id;
...
//                        }
                ...
                Session::flash('class', $class);
                Session::flash('display', 'block');
                if ($payment_task && $payment_task->is_api) {
                    if ($status == "page-success") {
                        return redirect("asercargo://app/payment/success");
                    } else {
                        return redirect("asercargo://app/payment/error");
