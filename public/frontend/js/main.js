...
function change_price_for_special_product (number) {
  ...
  let check_multi_price = price * qty;
  ...
  let checkPackage = total_price; 
  
if (check_multi_price > 700) {
    alert('Bir məhsulun dəyəri 700 ABŞ dollarından yuxarı olan sifarişlər qəbul olunmur.');

    $('#price_' + number).val(''); 
    $('#quantity_' + number).val('');
    
    $('#total_price_' + number).html('0.00' + currency_name).attr('price', 0);

    let total_price = 0;
    jQuery('.total_price').each(function () {
        let current_price = parseFloat($(this).attr('price'));
        if (isNaN(current_price)) {
            current_price = 0;
        }
        total_price += current_price; 
    ...
    $('#total_price_span').html(total_price + ' ').attr('price', total_price);

    return; 
} else {
    let total_with_commission = check_multi_price + (check_multi_price * special_orders_percent) / 100;
    total_with_commission = total_with_commission.toFixed(2);

    $('#total_price_' + number).html(' ' + total_with_commission + ' ' + currency_name).attr('price', total_with_commission);
...
function new_special_product(e, number) {
  let new_number = $('.sp-product-block').length + 1;

  let new_btn_title = $(e).attr('data-title');
  ...
  let url_label = $('#url_label').text();
  let size_label = $('#size_label').text();
  let color_label = $('#color_label').text();
  let quantity_label = $('#quantity_label').text();
  let order_button_text = $('#new_order_button').text();
  let remove_order_button = $('#remove_order_button_text').text();
  let price_label = $('#price_label').text();
  let description_label = $('#description_label').text();
  let new_order_button_text = $('#new_order_button').attr('new_order_button_text') || order_button_text;
  let remove_order_button_text = $('#new_order_button').attr('remove_order_button_text') || remove_order_button;
  //console.log(new_order_button_text);
  $('#product_button_' + number).html(new_button);

  currency_name = $('#total_price_span').attr('currency_name');
  ...
  new_order += '</div>';
  new_order += '<div class="seperate_block col-12 row">';
  new_order += '<div class="col-md-6 col-xs-12 n-margin">';
  new_order += '<div class="input_custom input-effect">';
  ...
  new_order += '</div>';
  new_order += '<div class="seperate_block col-12 row">';
  new_order += '<div class="col-md-3 col-xs-12 n-margin">';
  new_order += '<div class="input_custom input-effect">';
  new_order += '<input class="effect-20" type="number" placeholder="" name="quantity[]" id="quantity_'  + new_number + '" oninput="change_price_for_special_product(' + new_number + ');" min="0.01">';
  new_order += '<label>' + quantity_label + '</label>';
  new_order += '<span class="focus-border"><i></i></span>';
  new_order += '</div>';
  new_order += '</div>';
  new_order += '<div class="col-md-5 col-xs-12 n-margin">';
  new_order += '<div class="input_custom input-effect">';
  ...
  new_order += '</div>';
  new_order += '</div>';
  new_order += '<div class="col-md-4 col-xs-12 n-margin">';
  ...
  new_order += '<div class="sp-product-control btn btn-add btn-success" onclick="new_special_product(this, ' + new_number + ');" data-title="' + remove_order_button_text + ' <i class=&quot;fa fa-minus&quot;></i>">';
  new_order += new_order_button_text + ' <i class="fa fa-plus"></i>';
  new_order += '</div>';
  new_order += '</div>';
  new_order += '</div>';
...
function remove_special_product(number) {
  let price = parseFloat($('#total_price_' + number).attr('price'));
  let total_price = parseFloat($('#total_price_span').attr('price'));
  ...
  $('#product_block_' + number).remove();


  $('#total_price_span').html(new_price + ' ' + $('#total_price_span').attr('currency_name')).attr('price', new_price);
}
