    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.case === 'success') {
                ...
                // Ödeme butonlarını aktif hale getirme işlemi kaldırıldı
...
function courier_choose_packages_modal(url) {
    ...
    show_modal('modalProfileCurierOrders');
}
...
$("#checkAll").click(function () {
    const isChecked = this.checked !== undefined ? this.checked : this.getElementsByTagName('input')[0].checked;
    if (this.checked === undefined){
        this.getElementsByTagName('input')[0].checked = !isChecked;
    }
    $('.checks').not(this).prop('checked', !isChecked);
});

$(".checks").click(function (){
    const isChecked = this.checked;
    $(".checks").checked = !isChecked;
    const checkedCheckboxSize= $('.checks:checked').length;
    console.log($("#checkAll input")[0]);
    $("#checkAll input")[0].checked = $(".checks").length === checkedCheckboxSize;
});

$(".checks_region").click(function (){
    const isChecked = this.checked;
    $(".checks_region").checked = !isChecked;
    const checkedCheckboxSize= $('.checks_region:checked').length;
    ...
    $("#category_id").change(function (){
        ...
        for (let i = 0; i < options.length; i++) {
            if (options[i].value === categoryId){
                categoryText = options[i].innerText;
                $("#title").val(categoryText);
                break;
            }
        }
    });
...
function check_packages_region() {
    ...
    $.ajax({
        type: "get",
        url: "/az/account/courier/get-courier-payment-types-regions",
        data: {
            '_token': CSRF_TOKEN,
        },
        success: function (response) {
